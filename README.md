...
This program is designed for the incorporating mutation spectrum into the Nei-Gojobori method (Nei and Gojobori, 1986) by using the relative rates of six mutation types (AT -> TA, AT -> CG, AT -> GC, CG -> TA, CG -> AT, CG -> GC) to calculate S and N for a sequence. This program is written in Python and requires Python >= 3.10.
...
These numbers could be actual mutation rate * some arbitrary constant, as long as these 6 parameters are in the same scale.
...
These numbers could be actual mutation rate * some arbitrary constant, as long as:
 a, b, and c should be in the same scale. 
 d, e, and f should be in the same scale.
 g and h should be in the same scale.
...
Please note that labels should be consistent with indicated in the example. The order of labels does not matter.
These numbers could be actual mutation rate * some arbitrary constant, as long as:
 Relative mutation rate of different point mutations from A/T should be in the same scale. 
 Relative mutation rate of different point mutations from C/G should be in the same scale.
 Mutation rate of A/T and C/G should be in the same scale.
